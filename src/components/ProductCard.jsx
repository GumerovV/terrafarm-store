...
function ProductCard({item}) {
    return ( 
        <Link to={`${item.id}`}>
            <div className="group font-extralight inline-block relative truncate no-underline mt-4 mr-1.5 mb-0 ml-1.5 p-5 w-72 h-72" key={item.id}>
                    <img className="max-w-full w-full h-full duration-0.2 delay-0 transition-opacity rounded-lg mt-6" src={item.img} alt=""></img>
                    <div class="absolute inset-0 flex-col opacity-100 bg-black w-54 h-56 mt-64 group-hover:opacity-95 group-hover:mt-16 ml-5 mr-5 duration-500">
